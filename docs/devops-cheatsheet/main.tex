...
Docker, kubectl, Helm and Terraform are all single binaries with a package in
every mainstream package manager. Install them that way rather than by hand:
the manager puts the binary on your \texttt{PATH} and upgrades it later.

\subsection{Windows}

\texttt{winget} ships with Windows 11 and recent Windows 10 builds. Chocolatey
(\texttt{choco install}) and Scoop (\texttt{scoop install}) carry the same
packages if you already use one.
...
\field{Close and reopen the terminal afterwards.}{A running shell keeps the
\texttt{PATH} it started with, so a freshly installed binary is not found until
you open a new one. Docker Desktop also has to be launched once before
\texttt{docker} will talk to anything.}

\subsection{macOS}

Homebrew covers all four. The \texttt{-{}-cask} form installs the Docker Desktop
application; use \texttt{brew install docker} instead if you only want the CLI
against a remote or Colima daemon.
...
Docker Engine, via the convenience script. The \texttt{usermod} line is what
lets you run \texttt{docker} without \texttt{sudo}, and it only takes effect
...
kubectl, pinned to whatever the current stable release is:
...
Helm, via the official installer script. Read it before running it on a machine
that matters: it installs a binary into \texttt{/usr/local/bin} with
\texttt{sudo}.
...
Terraform, from the HashiCorp apt repository on Debian and Ubuntu. On the RHEL
family use \texttt{dnf config-manager} with
\texttt{https://rpm.releases.hashicorp.com}.
...
\field{Snap works too}{for Helm (\texttt{snap install helm -{}-classic}) and
kubectl (\texttt{snap install kubectl -{}-classic}), if your distribution
already has snapd.}

\subsection{Verify}

\begin{cmdtable}[5.6cm]
docker -{}-version & Prints the Engine version. "Cannot connect to the Docker daemon" means Docker is installed but not running \nr
kubectl version -{}-client & Prints the client version without needing a cluster. Drop \texttt{-{}-client} to check the cluster connection as well \nr
helm version & Prints \texttt{version.BuildInfo}. Helm 3 has no server component, but it reads your kubeconfig, so fix \texttt{kubectl} first \nr
terraform -version & Prints the version and warns when a newer one is out \\
\end{cmdtable}
